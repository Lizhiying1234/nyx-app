...
/**
 * ★★ 这一次调用**是不是 DeepSeek** · D-496（2026-09-16）
 *
 * ── 为什么需要这个函数 ────────────────────────────────────────
 * 上面那份 `PROVIDERS` 是**设置页的快填清单**，调用层从不查它（见它自己的头注），
 * provider id 也**从不落库** —— 库里只有 `ai.<slot>.baseUrl` · `.model` · `.protocol` 三样。
 * 也就是说：**运行时根本没有「当前 Provider 是谁」这个事实**，它只能被判出来。
 * D-496 要的「上层只说『这是 DeepSeek』」，落到代码里就是这个函数。
 *
 * ── 判据（使用者划的线：模型名不能决定关不关思考）──────────────
 *   ① 官方直连：host 是 `deepseek.com` 或它的子域 → `deepseek`
 *      两个口都算（`/v1` OpenAI 兼容 · `/anthropic` Anthropic 兼容），**与模型名无关**。
 *   ② 经聚合器：host 是 `openrouter.ai` 且模型**路由前缀**是 `deepseek/` → `deepseek-openrouter`
 *      ★ 这一条是主控 2026-09-16 裁的「乙」，理由写在明处：OpenRouter 的 `{author}/{slug}`
 *        里那个 author 段**就是 Provider**，不是模型名 —— 换 `deepseek/` 下的任何模型都成立，
 *        所以它不违反「不许按模型名分支」。**使用者待复核**（D-496 附记）。
 *        不这么判的话，他哪天把那一格换成快填清单里现成的
 *        `deepseek/deepseek-v4-pro-20260813`，思考就会原样回来。
 *   ③ 其余一律 `unknown` —— **判不出来就什么都不加**（D-496「不支持不强发」）。
 *
 * ★ host 用「相等或以 `.域名` 结尾」比，不用 `includes`：
 *   `includes('deepseek.com')` 会把 `api.deepseek.com.somethingelse.net` 也认成 DeepSeek。
 */
export type AiVendor = 'deepseek' | 'deepseek-openrouter' | 'unknown'

/** 取 host。拿不出来就空串（调用方据此当 unknown）。★ 反斜杠绕开写（D-460）：用 `[/]` 不用转义 */
...
const hostIs = (host: string, domain: string): boolean =>
  host === domain || host.endsWith('.' + domain)

export function providerOf(baseUrl: string, model: string): AiVendor {
  const host = hostOf(baseUrl)
  if (!host) return 'unknown'
  if (hostIs(host, 'deepseek.com')) return 'deepseek'
  if (hostIs(host, 'openrouter.ai') && model.trim().toLowerCase().startsWith('deepseek/')) {
