 ...
 * ★★ 记一笔（不是弹窗）· D-496 W-4
 *
 * core 里没有日志出口 —— 打包后 `console.*` 无处可看（`core/sync/problems.ts` 头注记过这件事）。
 * 所以按 `main/tts.ts` 那个先例：**由端把自己那支笔交进来**，默认是空操作。
 * Windows 在 `main/index.ts` 调一次 `setAiLog(log)`；Android 接自己的（那半不在本轮）。
 */
let aiLog: (level: 'info' | 'error', where: string, msg: string) => void = () => {}
export const setAiLog = (fn: typeof aiLog): void => {
  aiLog = fn
}

/**
 * ★★ 判定为 DeepSeek 就默认关思考 · D-496（2026-09-16）
 *
 * ── 为什么在这一层 ───────────────────────────────────────────
 * 使用者原话：「上层只需要说『这是 DeepSeek』，Provider 层负责确保它以关闭 thinking 的方式调用」。
 * 这个文件是两端**唯一**的请求出口（Windows 11 个调用点 · Android 5 个都经过这里），
 * 所以「唯一那道口子」只能是这里。上层一个字都不用改，也**不许**由上层决定思不思考。
 *
 * ── 字段按 **API 兼容形状**分，不按模型名分（使用者划的线）──────
 *   DeepSeek 官方 · OpenAI 兼容口 `/v1`        → `thinking: { type: 'disabled' }`
 *   DeepSeek 官方 · Anthropic 兼容口 `/anthropic` → `thinking: { type: 'disabled' }`（同名字段）
 *   OpenRouter 上的 `deepseek/…`               → `reasoning: { enabled: false, effort: 'none' }`
 *   其余                                        → **一个字段都不加**
 *
 * ★ 出处（2026-09-16 现查，不是记忆；下次核对逐条重开这些 URL）：
 *   · https://api-docs.deepseek.com/api/create-chat-completion
 *     顶层 `thinking` 对象：`type: "enabled" | "disabled"`，**默认 `enabled`**；
 *     另有子字段 `reasoning_effort: none|low|high|max`。
 *   · https://api-docs.deepseek.com/guides/anthropic_api
 *     Anthropic 兼容口在 `https://api.deepseek.com/anthropic`，兼容表里 `thinking`
 *     一栏写的是「Supported（`budget_tokens` 被忽略）」—— **所以这一口也是 `thinking`**。
 *     ★ 派单原写「这一口用 `reasoning:{effort:'none'}`」，查文档核出来不是，按文档落（已报主控）。
 *   · https://openrouter.ai/docs/use-cases/reasoning-tokens
 *     `reasoning` 对象有 `enabled`（布尔）· `effort`（含 `'none'`）· `max_tokens` · `exclude`。
 *     这里**两键同发** `{ enabled: false, effort: 'none' }`：
 *       ☆ 文档说 `effort: 'none'` 就是关，但 D 实地报告它在 OpenRouter + DeepSeek 上**不总生效**；
 *       ☆ `enabled: false` 是文档里的显式开关。
 *       ★ 说在明处：文档只禁 `effort` 与 `max_tokens` 同发（"one of the following (not both)"），
 *         对 `enabled` + `effort` **没有明文**，还暗示 `enabled` 是简写、不宜与 `effort` 共存 ——
 *         所以两键同发是**我们在文档留白处下的注**（主控 2026-09-16 裁），不是文档背书。
 *         真被某一版拒掉，下面那道 400 兜底会去掉它重试一次并记一笔。
 *
 * ★★ **`exclude` 一个字都不许发**（闸盯着）。`reasoning: { exclude: true }` 是
 *   「照样想、只是不回给你」：额度一分不省 —— 而这次要治的病正是「额度全被思考吃光」。
 *   还有更坏的一层：响应里没有 `reasoning_content` 了，`reasoningChars()` 归 0，
 *   于是 09-13 那句「模型把额度全用在思考上了」的诊断会**退化回「可能是模型名不对」**，
 *   把他往改一个本来就对的模型名上带（正是那次要修的假话）。
 *
 * ★ 只发一个字段（不带 `reasoning_effort`）：DeepSeek 自家两页文档对它放顶层还是放
 *   `thinking` 里说法不一致；字段越少，被某个网关拒掉的机会越少。主控 2026-09-16 裁。
 *
 * ★ `temperature` 不在这里另定：关掉思考之后它照常生效，**那正是本意**
 *   （判分/分析传 0 要的就是可复现，见 `CallOptions.temperature`）。
 */
function noThinking(cfg: SlotConfig, protocol: Protocol): Record<string, unknown> {
  const vendor = providerOf(cfg.baseUrl, cfg.model)
  if (vendor === 'deepseek') {
    // 两个官方口同名字段；gemini 协议上没有 DeepSeek，判不出来就不加
    if (protocol === 'openai' || protocol === 'anthropic') return { thinking: { type: 'disabled' } }
    return {}
  }
  if (vendor === 'deepseek-openrouter') return { reasoning: { enabled: false, effort: 'none' } }
  return {}
}
...
function shape(cfg: SlotConfig, opts: CallOptions, protocol: Protocol): Shaped {
  ...
  if (protocol === 'anthropic') {
    return {
      ...
      body: {
        ...
        ...noThinking(cfg, protocol), // D-496 · 判定为 DeepSeek 才加，其余一个字段都不多
  ...
  return {
    ...
    body: {
      ...
      max_tokens: maxTokens,
      ...(temp === undefined ? {} : { temperature: temp }),
      ...noThinking(cfg, protocol), // D-496 · 同上：非 DeepSeek 一个字段都不多
...
export async function callAi(cfg: SlotConfig, opts: CallOptions, slot: Slot): Promise<string> {
  ...
  /**
   * ★★ 兜底：服务商拒绝我们加的那个「关思考」字段 → **去掉它重试一次**（D-496，主控 2026-09-16 点头）
   *
   * 两家文档都没写「发了不支持的字段会怎样」。判定为 DeepSeek 才加，所以正常路径上不该撞；
   * 会撞的是**说不准的那一类**：某个自建网关、某一版 OpenRouter、哪天官方改名。
   * 撞上时的正确行为不是「让他看见一个 400」——**他没做错任何事**，
   * 是我们多发了一个字段。所以：去掉那一个字段原样重试一次，**只重一次**。
   *
   * ★ 判据窄一点：只认 **400** 且**错误正文里点了那个字段的名字**。
   *   放宽成「任何 4xx 都重试」会把真正的 key 错 / 模型名错也重发一遍，
   *   多花一次钱，还把失败时间拖长一倍。
   * ★ 记一笔而不是弹窗（W-4）：这是**我们这边**的兼容问题，不是他要处理的事。
   */
  const dropKey = Object.keys(noThinking(cfg, protocol))[0]
  if (!res.ok && res.status === 400 && dropKey && text.includes(dropKey)) {
    const retryBody = { ...(s.body as Record<string, unknown>) }
    delete retryBody[dropKey]
    aiLog(
      'info',
      'ai',
      `${new URL(s.url).host} 拒绝了「关闭思考」那个字段（${dropKey}），` +
        `已去掉它重试一次 —— 这一次调用的思考没关掉。原文：${text.slice(0, 200)}`
    )
    const ac2 = new AbortController()
    const timer2 = setTimeout(() => ac2.abort(), opts.timeoutMs ?? 120_000)
    opts.signal?.addEventListener('abort', () => ac2.abort())
    try {
      res = await fetch(s.url, {
        method: 'POST',
        headers: s.headers,
      ...
      // 重试本身失败就不再救 —— 保持第一次那份 400 的原文，下面照常报
    } finally {
      clearTimeout(timer2)
    }
  }
