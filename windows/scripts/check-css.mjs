/**
 * CSS 注释闸 —— **一类 check:types 永远抓不到的 bug**（2026-09-01）
 *
 * ── 它抓的是什么 ────────────────────────────────────────────
 *
 * `enhancements.css` 里有过这么一行注释：
 *
 *     全部用总原型已有的令牌（--fs-<星><斜杠>--accent/--serif/--pad-work），
 *
 * 那两个字符正好是注释结束符。注释在那里就断了，后半句变成裸文本，
 * 被解析器当成选择器的一部分吞掉 —— **紧跟其后的整条规则从来没有生效过**。
 * 不报错、不警告、svelte-check 全绿，只是那一页的版式一直是浏览器默认值。
 *
 * ── 为什么不能「数符号个数」★★ ──────────────────────────────
 *
 * 第一版这个脚本就是数 `/*` 和 `*` `/` 的个数是否相等。**不够。**
 * 写修复说明的时候我在注释里引用了这两个符号，个数照样相等，
  ...
  lines.forEach((l, i) => {
    ...
    while (j < l.length) {
      ...
      if (inC && l.startsWith(OPEN, j)) {
        // 注释里又写了开始符：本身不致命，但下一个结束符会提前关掉注释
        problems.push(`第 ${i + 1} 行：注释（第 ${openedAt} 行开的）里又出现了开始符 —— CSS 注释不嵌套`)
        j += 2; continue
      }
...
console.log(`✓ ${DIR} 的 CSS 注释按 CSS 规则扫过，没有提前关掉的`)
