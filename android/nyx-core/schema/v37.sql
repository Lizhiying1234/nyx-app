...
-- ── 它是什么 ────────────────────────────────────────────────
--
--   migrations.ts    老库 → v37        （升级路径，只增不删）
--   本文件            v37 应该长什么样  （目标定义，两端共用）
--
...
-- ── Android 怎么用 ──────────────────────────────────────────
--
-- 全新安装**不重放历史 migration**，直接执行这一份建到 v37。
-- 之后的结构变更（v38 起）两端共用同一份 DDL。
...
CREATE TABLE answers (
        ...
        created_at  integer not null,
        updated_at  integer not null
      , uid text, device text, duration_ms integer);
...
CREATE TABLE item_lectures (
        ...
        created_at integer not null,
        updated_at integer not null, uid text, deleted_at integer,
...
CREATE TABLE sessions (
        ...
        created_at  integer not null,
        updated_at  integer not null
      , uid text, device text, rules text);
