 ...
 *
 * ★★ **本仓这一份是从 Windows 仓 `scripts/lib/strip-comments.mjs` 逐字抄来的**
 *   （ZA-1 · 2026-09-15；正则那个洞的修复同日逐字跟过来）。两仓各存一份、
 *   **行为必须相同** —— 不能 import 是因为两个仓之间只有 `nyx-core` 那一条
 *   submodule 通道，而这是工具不是判据。
 *   ☞ 改这份的那天，**两边一起改**；只改一边的后果是同一段源码在两仓被扫成两样，
 *     而不会有任何东西报错。
 *   ★ 这一条当天就兑现了一次：B 在 Windows 侧挖出「不认正则字面量」那个洞，
 *     本仓这份**一模一样地漏着** —— 两仓同形的好处是修法也能逐字跟，
 *     坏处是洞也一份不落。所以「两边一起改」不是客套话。
